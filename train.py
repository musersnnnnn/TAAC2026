...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--valid_metric', type=str, default='target_auc',
                        choices=[
                            'all_auc', 'target_auc', 'hour_auc', 'weekday_auc',
                            'online_long_auc', 'online_proxy_auc',
                        ],
                        help='Metric used for checkpoint selection / early stopping')
    parser.add_argument('--valid_target_hours', type=str, default='7,8,9',
                        help='UTC+8 target hours for sample-level validation metrics')
    parser.add_argument('--valid_target_weekdays', type=str, default='0',
                        help='UTC+8 target weekdays for sample-level validation metrics, Monday=0')
    parser.add_argument('--valid_target_min_samples', type=int, default=1000,
                        help='Fallback to all_auc when target validation sample count is smaller')
    ...
    parser.add_argument('--label_smoothing', type=float, default=0.0,
                        help='Binary label smoothing used by BCE training loss')
    parser.add_argument('--logit_l2', type=float, default=0.0,
                        help='Small L2 penalty on logits to reduce over-confident tails')
    parser.add_argument('--rank_loss_weight', type=float, default=0.0,
                        help='Weight of in-batch pairwise AUC surrogate loss')
    parser.add_argument('--rank_loss_temperature', type=float, default=1.0,
                        help='Temperature for pairwise AUC surrogate loss')
    parser.add_argument('--rank_loss_max_pairs', type=int, default=8192,
                        help='Maximum sampled positive-negative pairs per batch for rank loss')
    parser.add_argument('--sample_weight_mode', type=str, default='none',
                        choices=['none', 'target_time', 'long_history', 'online_proxy'],
                        help='Optional training sample weights for online-like slices')
    parser.add_argument('--tail_residual_scale', type=float, default=0.0,
                        help='Scale of the gated residual logit calibration head (0 = disabled)')
    parser.add_argument('--tail_aux_loss_weight', type=float, default=0.0,
                        help='Extra BCE weight on target/long/high-score samples for tail calibration')
    parser.add_argument('--tail_residual_l2', type=float, default=0.0,
                        help='L2 penalty on the gated residual logit contribution')
    parser.add_argument('--tail_high_score_quantile', type=float, default=0.80,
                        help='Batch prediction quantile used to select high-score tail samples')
    ...
    parser.add_argument('--dense_weight_decay', type=float, default=0.01,
                        help='Weight decay for dense AdamW parameters; bias/Norm params are excluded')
    parser.add_argument('--dense_ema_decay', type=float, default=0.0,
                        help='EMA decay for dense AdamW parameters only (0 = disabled)')
    parser.add_argument('--dense_ema_start_step', type=int, default=0,
                        help='Global train step at which dense-parameter EMA starts updating')
    ...
    parser.add_argument('--user_dense_tokens', type=int, default=1,
                        help='Number of chunked user dense tokens')
    parser.add_argument('--item_dense_tokens', type=int, default=1,
                        help='Number of chunked item dense tokens')
...
def main() -> None:
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        rank_loss_max_pairs=args.rank_loss_max_pairs,
        sample_weight_mode=args.sample_weight_mode,
        tail_aux_loss_weight=args.tail_aux_loss_weight,
        tail_residual_l2=args.tail_residual_l2,
        tail_high_score_quantile=args.tail_high_score_quantile,
        sparse_lr=args.sparse_lr,
        sparse_weight_decay=args.sparse_weight_decay,
        dense_weight_decay=args.dense_weight_decay,
        dense_ema_decay=args.dense_ema_decay,
        dense_ema_start_step=args.dense_ema_start_step,
        reinit_sparse_after_epoch=args.reinit_sparse_after_epoch,
        reinit_cardinality_threshold=args.reinit_cardinality_threshold,
        valid_metric=args.valid_metric,
        valid_target_hours=args.valid_target_hours,
        valid_target_weekdays=args.valid_target_weekdays,
        valid_target_min_samples=args.valid_target_min_samples,
