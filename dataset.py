...
ENGINEERED_CURRENT_TIME_FID = 920001
ENGINEERED_CURRENT_TIME_DIM = 6
ENGINEERED_SEQ_TIME_BASE_FID = 920101
ENGINEERED_SEQ_TIME_DIM = 6
ENGINEERED_SEQ_MATCH_BASE_FID = 920201
ENGINEERED_SEQ_MATCH_DIM = 16
ENGINEERED_USER_DENSE_STATS_FID = 920301
ENGINEERED_USER_DENSE_STATS_DIM = 32
DENSE_STATS_NUM_CHUNKS = 8
DENSE_STATS_SUM_SCALE = 1_000_000_000.0
DENSE_STATS_CHUNK_SUM_SCALE = 100_000_000.0
ENGINEERED_CURRENT_TIME_INT_BASE_FID = 930001
ENGINEERED_CURRENT_TIME_INT_SPECS = (
    (ENGINEERED_CURRENT_TIME_INT_BASE_FID + 0, 24, 1),   # UTC+8 hour: 1..24
    (ENGINEERED_CURRENT_TIME_INT_BASE_FID + 1, 7, 1),    # UTC+8 weekday: 1..7
    (ENGINEERED_CURRENT_TIME_INT_BASE_FID + 2, 168, 1),  # weekday-hour cross: 1..168
)
...
class PCVRParquetDataset(IterableDataset):
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._current_time_int_offset = self.user_int_schema.total_dim
        for fid, vs, dim in ENGINEERED_CURRENT_TIME_INT_SPECS:
            self.user_int_schema.add(fid, dim)
            self.user_int_vocab_sizes.extend([vs] * dim)
        ...
        self._original_user_dense_dim = self.user_dense_schema.total_dim
        self._user_dense_stats_offset = self.user_dense_schema.total_dim
        self.user_dense_schema.add(ENGINEERED_USER_DENSE_STATS_FID, ENGINEERED_USER_DENSE_STATS_DIM)
        ...
        self._current_time_dense_offset = self.user_dense_schema.total_dim
        self.user_dense_schema.add(ENGINEERED_CURRENT_TIME_FID, ENGINEERED_CURRENT_TIME_DIM)
        self._seq_time_dense_offsets = {}
        self._seq_match_dense_offsets = {}
        for i, domain in enumerate(self.seq_domains):
            self._seq_time_dense_offsets[domain] = self.user_dense_schema.total_dim
            self.user_dense_schema.add(ENGINEERED_SEQ_TIME_BASE_FID + i, ENGINEERED_SEQ_TIME_DIM)
            self._seq_match_dense_offsets[domain] = self.user_dense_schema.total_dim
            self.user_dense_schema.add(ENGINEERED_SEQ_MATCH_BASE_FID + i, ENGINEERED_SEQ_MATCH_DIM)
    ...
    @staticmethod
    def _log_norm(values: "npt.NDArray[np.float64]", scale: float) -> "npt.NDArray[np.float32]":
        scale = max(float(scale), 1.0)
        clipped = np.maximum(values.astype(np.float64), 0.0)
        return (np.log1p(clipped) / np.log1p(scale)).astype(np.float32)

    @staticmethod
    def _current_time_parts(
    ...
    def _fill_current_time_int(
        self,
        user_int: "npt.NDArray[np.int64]",
        timestamps: "npt.NDArray[np.int64]",
    ) -> None:
        offset = self._current_time_int_offset
        hours, weekdays = self._current_time_parts(timestamps)
        user_int[:, offset + 0] = hours + 1
        user_int[:, offset + 1] = weekdays + 1
        user_int[:, offset + 2] = weekdays * 24 + hours + 1

    def _fill_user_dense_stats(self, user_dense: "npt.NDArray[np.float32]") -> None:
        raw_dim = int(getattr(self, "_original_user_dense_dim", 0))
        if raw_dim <= 0:
            return

        offset = self._user_dense_stats_offset
        stats = user_dense[:, offset:offset + ENGINEERED_USER_DENSE_STATS_DIM]
        stats[:] = 0.0

        raw = np.maximum(user_dense[:, :raw_dim].astype(np.float64, copy=False), 0.0)
        nonzero = raw > 0.0
        nonzero_count = nonzero.sum(axis=1).astype(np.float64)
        dim_f = float(raw_dim)
        total = raw.sum(axis=1)
        max_val = raw.max(axis=1)
        l2_norm = np.sqrt(np.square(raw).sum(axis=1))
        mean_nonzero = np.divide(
            total,
            np.maximum(nonzero_count, 1.0),
            out=np.zeros_like(total, dtype=np.float64),
            where=nonzero_count > 0,
        )

        stats[:, 0] = self._log_norm(total, DENSE_STATS_SUM_SCALE)
        stats[:, 1] = self._log_norm(max_val, DENSE_STATS_SUM_SCALE)
        stats[:, 2] = self._log_norm(l2_norm, DENSE_STATS_SUM_SCALE)
        stats[:, 3] = (nonzero_count / dim_f).astype(np.float32)
        stats[:, 4] = self._log_norm(mean_nonzero, 10_000_000.0)
        stats[:, 5] = (max_val / np.maximum(total, 1.0)).astype(np.float32)
        stats[:, 6] = ((raw > 1_000.0).sum(axis=1).astype(np.float64) / dim_f).astype(np.float32)
        stats[:, 7] = ((raw > 100_000.0).sum(axis=1).astype(np.float64) / dim_f).astype(np.float32)

        chunk_edges = np.linspace(0, raw_dim, DENSE_STATS_NUM_CHUNKS + 1, dtype=np.int64)
        for chunk_idx in range(DENSE_STATS_NUM_CHUNKS):
            start, end = int(chunk_edges[chunk_idx]), int(chunk_edges[chunk_idx + 1])
            if end <= start:
                continue
            chunk = raw[:, start:end]
            chunk_nonzero = chunk > 0.0
            base = 8 + chunk_idx * 3
            stats[:, base + 0] = self._log_norm(chunk.sum(axis=1), DENSE_STATS_CHUNK_SUM_SCALE)
            stats[:, base + 1] = self._log_norm(chunk.max(axis=1), DENSE_STATS_CHUNK_SUM_SCALE)
            stats[:, base + 2] = (
                chunk_nonzero.sum(axis=1).astype(np.float64) / float(end - start)
            ).astype(np.float32)
    ...
    def _fill_current_time_dense(
        ...
        offset = self._current_time_dense_offset
        hours_i, weekdays_i = self._current_time_parts(timestamps)
        hours = hours_i.astype(np.float64)
        weekdays = weekdays_i.astype(np.float64)
        user_dense[:, offset + 0] = np.sin(2.0 * np.pi * hours / 24.0).astype(np.float32)
        user_dense[:, offset + 1] = np.cos(2.0 * np.pi * hours / 24.0).astype(np.float32)
        user_dense[:, offset + 2] = np.sin(2.0 * np.pi * weekdays / 7.0).astype(np.float32)
        user_dense[:, offset + 3] = np.cos(2.0 * np.pi * weekdays / 7.0).astype(np.float32)
        user_dense[:, offset + 4] = (weekdays_i == 0).astype(np.float32)
        user_dense[:, offset + 5] = ((hours_i >= 7) & (hours_i <= 9)).astype(np.float32)

    def _fill_seq_time_dense(
        self,
        user_dense: "npt.NDArray[np.float32]",
        domain: str,
        ...
        offset = self._seq_time_dense_offsets[domain]
        valid = ts_padded > 0
        cnt = valid.sum(axis=1).astype(np.float64)
        has = cnt > 0
        large = np.int64(2**62 - 1)
        min_diff = np.where(valid, time_diff, large).min(axis=1).astype(np.float64)
        min_diff[~has] = 0.0
        sum_diff = np.where(valid, time_diff, 0).sum(axis=1).astype(np.float64)
        mean_diff = np.zeros_like(sum_diff, dtype=np.float64)
        mean_diff[has] = sum_diff[has] / cnt[has]
        max_ts = np.where(valid, ts_padded, 0).max(axis=1).astype(np.float64)
        min_ts = np.where(valid, ts_padded, large).min(axis=1).astype(np.float64)
        min_ts[~has] = 0.0
        span = np.maximum(max_ts - min_ts, 0.0)
        denom_len = max(float(max_len), 1.0)
        recent_1d = ((time_diff <= 86400) & valid).sum(axis=1).astype(np.float64)
        recent_7d = ((time_diff <= 604800) & valid).sum(axis=1).astype(np.float64)
        user_dense[:, offset + 0] = (lengths.astype(np.float32) / denom_len)
        user_dense[:, offset + 1] = self._log_norm(lengths.astype(np.float64), denom_len)
        user_dense[:, offset + 2] = self._log_norm(min_diff, 31536000.0)
        user_dense[:, offset + 3] = self._log_norm(mean_diff, 31536000.0)
        user_dense[:, offset + 4] = self._log_norm(span, 31536000.0)
        user_dense[:, offset + 5] = self._log_norm(recent_1d + recent_7d, 2.0 * denom_len)

    def _fill_seq_match_dense(
        self,
        user_dense: "npt.NDArray[np.float32]",
        domain: str,
        ...
        max_len: int,
        time_diff: Optional["npt.NDArray[np.int64]"] = None,
    ) -> None:
        """Add compact item-history match statistics for one sequence domain.

        These features intentionally stay as dense summary signals instead of
        changing the backbone. Values <= 2 are ignored to reduce accidental
        matches from padding and tiny categorical namespaces. The first ten
        slots are position-based for backward continuity; the last six are
        true time-delta match summaries, which are more robust when sequence
        order differs between local and online data.
        """
        offset = self._seq_match_dense_offsets[domain]
        B, n_feat, _ = seq_values.shape
        denom_len = max(float(max_len), 1.0)
        denom_feat = max(float(n_feat), 1.0)

        for i in range(B):
            seq_len = min(int(lengths[i]), max_len)
            if seq_len <= 0:
                continue

            target_values = item_int[i][item_int[i] > 2]
            if target_values.size == 0:
                continue
            target_values = np.unique(target_values)

            seq_slice = seq_values[i, :, :seq_len]
            seq_valid = seq_slice > 2
            if not seq_valid.any():
                continue

            matches = np.isin(seq_slice, target_values) & seq_valid
            per_event = matches.any(axis=0)
            total_matches = float(matches.sum())
            matched_events = float(per_event.sum())
            valid_cells = max(float(seq_valid.sum()), 1.0)
            seq_len_f = max(float(seq_len), 1.0)
            matched_features = float(matches.any(axis=1).sum())

            if matched_events > 0:
                first_match = int(np.argmax(per_event))
                last_match = int(seq_len - 1 - np.argmax(per_event[::-1]))
                has_match = 1.0
            else:
                first_match = seq_len
                last_match = seq_len
            ...
            weights = np.exp(-np.arange(seq_len, dtype=np.float64) / 32.0)
            recent_score = float((per_event.astype(np.float64) * weights).sum() / weights.sum())

            user_dense[i, offset + 0] = self._log_norm(np.array([total_matches]), valid_cells)[0]
            user_dense[i, offset + 1] = self._log_norm(np.array([matched_events]), seq_len_f)[0]
            user_dense[i, offset + 2] = has_match
            user_dense[i, offset + 3] = float(per_event[:1].sum())
            user_dense[i, offset + 4] = float(per_event[:min(seq_len, 10)].sum()) / min(seq_len_f, 10.0)
            user_dense[i, offset + 5] = float(per_event[:min(seq_len, 50)].sum()) / min(seq_len_f, 50.0)
            user_dense[i, offset + 6] = recent_score
            user_dense[i, offset + 7] = 1.0 - min(float(first_match), denom_len) / denom_len
            user_dense[i, offset + 8] = 1.0 - min(float(last_match), denom_len) / denom_len
            user_dense[i, offset + 9] = matched_features / denom_feat

            if time_diff is None or offset + 15 >= user_dense.shape[1]:
                continue
            diff_slice = time_diff[i, :seq_len].astype(np.float64, copy=False)
            matched_diffs = diff_slice[per_event]
            if matched_diffs.size == 0:
                continue
            matched_diffs = np.maximum(matched_diffs, 0.0)
            user_dense[i, offset + 10] = self._log_norm(
                np.array([matched_diffs.min()]), 31536000.0)[0]
            user_dense[i, offset + 11] = self._log_norm(
                np.array([matched_diffs.mean()]), 31536000.0)[0]
            user_dense[i, offset + 12] = float((matched_diffs <= 86400).sum()) / seq_len_f
            user_dense[i, offset + 13] = float((matched_diffs <= 604800).sum()) / seq_len_f
            user_dense[i, offset + 14] = float((matched_diffs <= 2592000).sum()) / seq_len_f
            user_dense[i, offset + 15] = float(np.exp(-matched_diffs / 604800.0).sum()) / seq_len_f
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        self._fill_current_time_int(user_int, timestamps)
        ...
        self._fill_user_dense_stats(user_dense)
        ...
        for domain in self.seq_domains:
            ...
            if ts_ci is not None:
                ...
                time_bucket[:] = buckets
                self._fill_seq_time_dense(user_dense, domain, lengths, ts_padded, time_diff, max_len)
                time_diff_for_match = time_diff

            self._fill_seq_match_dense(
                user_dense, domain, out, lengths, item_int, max_len, time_diff_for_match)
