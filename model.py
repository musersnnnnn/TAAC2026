...
class RoPEMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if sdpa_attn_mask is not None:
            # SDPA can produce NaN gradients when a query row has no valid key.
            # Let such rows attend everywhere; their sequence branch is residual-
            # guarded, and this avoids all-False softmax rows during backward.
            all_false = ~sdpa_attn_mask.any(dim=-1, keepdim=True)
            sdpa_attn_mask = sdpa_attn_mask.masked_fill(all_false, True)
...
class HashEmbedding(nn.Module):
    """Small hash embedding for sparse features skipped by cardinality."""

    def __init__(self, num_buckets: int, emb_dim: int) -> None:
        super().__init__()
        self.num_buckets = max(int(num_buckets), 2)
        self.emb = nn.Embedding(self.num_buckets, emb_dim, padding_idx=0)

    def _bucketize(self, values: torch.Tensor) -> torch.Tensor:
        vals = values.long()
        positive = vals > 0
        buckets = torch.zeros_like(vals)
        buckets[positive] = (vals[positive] % (self.num_buckets - 1)) + 1
        return buckets

    def forward(self, values: torch.Tensor) -> torch.Tensor:
        return self.emb(self._bucketize(values))


def _pooled_feature_embedding(
    emb_layer: nn.Module,
...
class GroupNSTokenizer(nn.Module):
    ...
    def __init__(self, feature_specs: List[Tuple[int, int, int]],
                 ...
                 use_hash_for_skipped_sparse: bool = False,
                 hash_bucket_size: int = 100000) -> None:
        super().__init__()
        self.feature_specs = feature_specs
        self.groups = groups
        self.emb_dim = emb_dim
        self.emb_skip_threshold = emb_skip_threshold
        self.use_hash_for_skipped_sparse = use_hash_for_skipped_sparse
        self.hash_bucket_size = hash_bucket_size

        # One embedding table per fid (None if skipped by emb_skip_threshold
        # or if vocab_size <= 0 / no vocab info). High-cardinality skipped
        # features can receive a small hash embedding instead of zeros.
        embs = []
        hash_embs = []
        for vs, offset, length in feature_specs:
            high_card_skip = emb_skip_threshold > 0 and int(vs) > emb_skip_threshold
            skip = int(vs) <= 0 or high_card_skip
            if skip:
                embs.append(None)
                if use_hash_for_skipped_sparse and high_card_skip:
                    hash_embs.append(HashEmbedding(hash_bucket_size, emb_dim))
                else:
                    hash_embs.append(None)
            else:
                embs.append(nn.Embedding(int(vs) + 1, emb_dim, padding_idx=0))
                hash_embs.append(None)
        self.embs = nn.ModuleList([e for e in embs if e is not None])
        self.hash_embs = nn.ModuleList([e for e in hash_embs if e is not None])
        # Map from fid index to position in self.embs/hash_embs (or -1 if unavailable)
        self._emb_index = []
        self._hash_index = []
        real_idx = 0
        hash_idx = 0
        for e, he in zip(embs, hash_embs):
            if e is not None:
            ...
            if he is not None:
                self._hash_index.append(hash_idx)
                hash_idx += 1
            else:
                self._hash_index.append(-1)
    ...
    def forward(self, int_feats: torch.Tensor) -> torch.Tensor:
        ...
        for group, proj in zip(self.groups, self.group_projs):
            ...
            for fid_idx in group:
                ...
                if emb_real_idx == -1:
                    hash_real_idx = self._hash_index[fid_idx]
                    if hash_real_idx == -1:
                        fid_emb = int_feats.new_zeros(int_feats.shape[0], self.emb_dim)
                    else:
                        fid_emb = _pooled_feature_embedding(
                            self.hash_embs[hash_real_idx], int_feats, offset, length)
                else:
                    fid_emb = _pooled_feature_embedding(
                        self.embs[emb_real_idx], int_feats, offset, length)
...
class ChunkDenseTokenizer(nn.Module):
    """Split a flat dense vector into a small fixed number of dense tokens."""

    def __init__(self, input_dim: int, d_model: int, num_tokens: int = 1) -> None:
        super().__init__()
        self.input_dim = int(input_dim)
        self.num_tokens = max(1, int(num_tokens))
        self.chunk_dim = math.ceil(max(self.input_dim, 1) / self.num_tokens)
        self.pad_dim = self.chunk_dim * self.num_tokens - self.input_dim
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.view(x.shape[0], self.num_tokens, self.chunk_dim)
        return self.proj(x)


class DINSequenceReader(nn.Module):
    """Target-aware DIN pooling over the four behavior sequences."""
    ...
    def __init__(
        ...
        self.domain_gate = nn.Sequential(
            nn.Linear(d_model * (num_sequences + 1), hidden),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, num_sequences),
        )
    ...
    def _pool_one(
        ...
        x = torch.cat([q, seq_tokens, q - seq_tokens, q * seq_tokens], dim=-1)
        scores = self.event_mlp(x).squeeze(-1)
        valid = ~padding_mask
        ...
        context = torch.bmm(weights.unsqueeze(1), seq_tokens).squeeze(1) / denom
        return context, valid.any(dim=-1)
    ...
    def forward(
        ...
        for seq_tokens, padding_mask in zip(seq_tokens_list, seq_padding_masks):
            ctx, valid = self._pool_one(query, seq_tokens, padding_mask)
            contexts.append(ctx)
            domain_valids.append(valid)

        ctx_stack = torch.stack(contexts, dim=1)  # (B, S, D)
        valid_stack = torch.stack(domain_valids, dim=1)  # (B, S)
        gate_in = torch.cat([query, ctx_stack.flatten(1)], dim=-1)
        gate_logits = self.domain_gate(gate_in).masked_fill(~valid_stack, -1.0e4)
        gate = torch.softmax(gate_logits, dim=-1) * valid_stack.float()
        gate = gate / gate.sum(dim=-1, keepdim=True).clamp_min(1.0e-6)
        fused = (ctx_stack * gate.unsqueeze(-1)).sum(dim=1)
        return self.out_proj(torch.cat([query, fused], dim=-1))
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        self.use_hash_for_skipped_sparse = use_hash_for_skipped_sparse
        self.hash_bucket_size = hash_bucket_size
        self.tail_residual_scale = float(tail_residual_scale)
        ...
        if ns_tokenizer_type == 'group':
            ...
            self.user_ns_tokenizer = GroupNSTokenizer(
                ...
                use_hash_for_skipped_sparse=use_hash_for_skipped_sparse,
        ...
        # Dense feature tokenization (if available). Baseline used one dense
        # token; this variant allows a small number of chunked dense tokens so
        # useful dense structure is not compressed too early.
        self.has_user_dense = user_dense_dim > 0
        self.num_user_dense_tokens = max(1, int(user_dense_tokens)) if self.has_user_dense else 0
        if self.has_user_dense:
            self.user_dense_proj = ChunkDenseTokenizer(
                user_dense_dim, d_model, self.num_user_dense_tokens)

        self.has_item_dense = item_dense_dim > 0
        self.num_item_dense_tokens = max(1, int(item_dense_tokens)) if self.has_item_dense else 0
        if self.has_item_dense:
            self.item_dense_proj = ChunkDenseTokenizer(
                item_dense_dim, d_model, self.num_item_dense_tokens)

        # Total NS token count
        self.num_ns = (num_user_ns + self.num_user_dense_tokens
                       + num_item_ns + self.num_item_dense_tokens)
        ...
        def _make_seq_embs(vocab_sizes):
            """Create embedding/hash-embedding lists for sequence features."""
            embs_raw = []
            hash_embs_raw = []
            for vs in vocab_sizes:
                ...
                if skip:
                    ...
                    if use_hash_for_skipped_sparse and high_card_skip:
                        hash_embs_raw.append(HashEmbedding(hash_bucket_size, emb_dim))
                    else:
                        hash_embs_raw.append(None)
                else:
            ...
            hash_module_list = nn.ModuleList([e for e in hash_embs_raw if e is not None])
            index_map = []
            hash_index_map = []
            real_idx = 0
            hash_idx = 0
            for e, he in zip(embs_raw, hash_embs_raw):
                if e is not None:
                    ...
                    index_map.append(-1)
                if he is not None:
                    hash_index_map.append(hash_idx)
                    hash_idx += 1
                else:
                    hash_index_map.append(-1)
            is_id = [int(vs) > seq_id_threshold for vs in vocab_sizes]
            return module_list, hash_module_list, index_map, hash_index_map, is_id
        ...
        self._seq_hash_index = {}   # domain -> hash index_map
        ...
        for domain in self.seq_domains:
            ...
            embs, hash_embs, idx_map, hash_idx_map, is_id = _make_seq_embs(vs)
            self._seq_embs[domain] = embs
            self._seq_hash_embs[domain] = hash_embs
            self._seq_emb_index[domain] = idx_map
            self._seq_hash_index[domain] = hash_idx_map
        ...
        # DIN reader: target-aware sequence pooling. The residual projection is
        # zero-initialized so training starts from the HyFormer baseline.
        self.din_reader = DINSequenceReader(
            d_model=d_model,
            num_sequences=self.num_sequences,
            hidden_mult=hidden_mult,
            dropout=dropout_rate,
        )
        self.din_residual = nn.Sequential(
            nn.Linear(d_model * 2, d_model),
            nn.LayerNorm(d_model),
            nn.SiLU(),
        ...
        nn.init.zeros_(self.din_residual[-1].weight)
        nn.init.zeros_(self.din_residual[-1].bias)
        ...
        # A small gated residual logit head for target-like / tail-like cases.
        # It is zero-initialized at the final layer, so the model starts exactly
        # from the v9 classifier and can only learn a bounded calibration term.
        self.tail_meta_dim = self.num_sequences * 3 + 4
        if self.tail_residual_scale > 0.0:
            self.tail_meta_proj = nn.Sequential(
                nn.Linear(self.tail_meta_dim, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
            )
            tail_input_dim = d_model * 3
            self.tail_gate = nn.Sequential(
                nn.Linear(tail_input_dim, d_model),
                nn.SiLU(),
                nn.Linear(d_model, action_num),
            )
            ...
            nn.init.constant_(self.tail_gate[-1].bias, -1.5)
            nn.init.zeros_(self.tail_residual_head[-1].weight)
            nn.init.zeros_(self.tail_residual_head[-1].bias)
        ...
        if emb_skip_threshold > 0:
            ...
            for name, tokenizer in [
                ...
                h = sum(1 for idx in getattr(tokenizer, '_hash_index', []) if idx != -1)
                t = len(tokenizer._emb_index)
                if f > 0:
                    logging.info(f"emb_skip_threshold={emb_skip_threshold}: {name} skipped {f}/{t} features, hash_filled={h}")
    ...
    def _init_params(self) -> None:
        ...
        for domain in self.seq_domains:
            for emb in self._seq_embs[domain]:
                ...
                emb.weight.data[0, :] = 0
            for hash_emb in self._seq_hash_embs[domain]:
                nn.init.xavier_normal_(hash_emb.emb.weight.data)
                hash_emb.emb.weight.data[0, :] = 0

        for tokenizer in [self.user_ns_tokenizer, self.item_ns_tokenizer]:
            for emb in tokenizer.embs:
                nn.init.xavier_normal_(emb.weight.data)
                emb.weight.data[0, :] = 0
            for hash_emb in getattr(tokenizer, 'hash_embs', []):
                nn.init.xavier_normal_(hash_emb.emb.weight.data)
                hash_emb.emb.weight.data[0, :] = 0
    ...
    def _embed_seq_domain(
        ...
        for i in range(S):
            ...
            if real_idx == -1:
                hash_real_idx = hash_index[i] if i < len(hash_index) else -1
                if hash_real_idx == -1:
                    emb_list.append(seq.new_zeros(B, L, self.emb_dim, dtype=torch.float))
                else:
                    e = hash_embs[hash_real_idx](seq[:, i, :])
    ...
    def _apply_din_residual(
        self,
        output: torch.Tensor,
        din_context: torch.Tensor,
    ) -> torch.Tensor:
        residual = self.din_residual(torch.cat([output, din_context], dim=-1))
        return output + residual

    def _tail_meta_features(self, inputs: ModelInput) -> torch.Tensor:
        """Build compact tail/profile features for the residual calibration head."""
        parts = []
        for domain in self.seq_domains:
            seq_len = inputs.seq_lens[domain].float()
            max_len = max(float(inputs.seq_data[domain].shape[2]), 1.0)
        ...
        if self.has_user_dense:
            dense_abs = inputs.user_dense_feats.float().abs()
            dense_dim = max(float(dense_abs.shape[1]), 1.0)
            dense_max = dense_abs.max(dim=1).values
            dense_mean = dense_abs.mean(dim=1)
            high_1k = (dense_abs > 1_000.0).float().sum(dim=1) / dense_dim
            high_100k = (dense_abs > 100_000.0).float().sum(dim=1) / dense_dim
            parts.extend([
                torch.log1p(dense_max).clamp(max=30.0) / 30.0,
                torch.log1p(dense_mean).clamp(max=25.0) / 25.0,
                high_1k.clamp(0.0, 1.0),
                high_100k.clamp(0.0, 1.0),
            ])
        else:
            batch_size = next(iter(inputs.seq_lens.values())).shape[0]
            zero = torch.zeros(batch_size, device=next(iter(inputs.seq_lens.values())).device)
            parts.extend([zero, zero, zero, zero])

        return torch.stack(parts, dim=-1)

    def _classify_with_tail_residual(
        self,
        output: torch.Tensor,
        din_context: torch.Tensor,
        inputs: ModelInput,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        base_logits = self.clsfier(output)
        if self.tail_residual_scale <= 0.0:
            zeros = torch.zeros_like(base_logits)
            return base_logits, base_logits, zeros, zeros

        meta = self._tail_meta_features(inputs)
        meta_emb = self.tail_meta_proj(meta)
        tail_input = torch.cat([output, din_context, meta_emb], dim=-1)
        gate = torch.sigmoid(self.tail_gate(tail_input))
        residual = self.tail_residual_head(tail_input)
        logits = base_logits + self.tail_residual_scale * gate * residual
        return logits, base_logits, residual, gate
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            user_dense_tok = F.silu(self.user_dense_proj(inputs.user_dense_feats))  # (B, Tu, D)
            ns_parts.append(user_dense_tok)
        item_parts = [item_ns]
        ns_parts.append(item_ns)
        if self.has_item_dense:
            item_dense_tok = F.silu(self.item_dense_proj(inputs.item_dense_feats))  # (B, Ti, D)
            item_parts.append(item_dense_tok)
            ns_parts.append(item_dense_tok)

        ns_tokens = torch.cat(ns_parts, dim=1)  # (B, num_ns, D)
        item_anchor = torch.cat(item_parts, dim=1).mean(dim=1)  # (B, D)
        ...
        for domain in self.seq_domains:
            tokens = self._embed_seq_domain(
                ...
                self._seq_embs[domain], self._seq_hash_embs[domain], self._seq_proj[domain],
                self._seq_is_id[domain], self._seq_emb_index[domain], self._seq_hash_index[domain],
                inputs.seq_time_buckets[domain])
            seq_tokens_list.append(tokens)
            mask = self._make_padding_mask(inputs.seq_lens[domain], inputs.seq_data[domain].shape[2])
            seq_masks_list.append(mask)

        # 3. Target-aware DIN pooling over raw sequence tokens
        din_context = self.din_reader(item_anchor, seq_tokens_list, seq_masks_list)

        # 4. Generate independent Q tokens per sequence via MultiSeqQueryGenerator
        ...
        # 5. Late-fuse HyFormer output with DIN residual, then classify
        output = self._apply_din_residual(output, din_context)
        logits, _, _, _ = self._classify_with_tail_residual(output, din_context, inputs)
        return logits

    def forward_with_tail_aux(
        self, inputs: ModelInput
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Forward pass returning final/base/residual/gate logits for training."""
        ...
        item_anchor = torch.cat(item_parts, dim=1).mean(dim=1)
        ...
        output = self._apply_din_residual(output, din_context)
        return self._classify_with_tail_residual(output, din_context, inputs)
